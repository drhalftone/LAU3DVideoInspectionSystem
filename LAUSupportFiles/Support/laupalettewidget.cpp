...
void LAUPaletteWidget::onError(QString string)
{
    if (this->isVisible()) {
        ...
        QMessageBox::warning(nullptr, QString("Palette Error"), string);
    }
}
...
LAUPalette::~LAUPalette()
{
    ...
    // DELETE SERIAL PORT OBJECT IF IT EXISTS
    if (port) {
        port->close();
        delete port;
    }
...
#else
        errorString = QString("No valid serial port.\n");
#endif
        emit emitError(errorString);
    }
    return (false);
...
/****************************************************************************/
/****************************************************************************/
bool LAUPalette::disconnectPort()
{
    if (port) {
        port->close();
        return (true);
