...
void main()
{
    ...
    float depthA = texelFetch(qt_depthTexture, ivec2(col, textureCoordinate.y), 0)[chn]; // * 7084.50 - 40.0;
    ...
    vec4 pixelA = texelFetch(qt_lutTexture, ivec3(textureCoordinate.x, textureCoordinate.y, int(floor(qt_layers * depthB))), 0);
    vec4 pixelB = texelFetch(qt_lutTexture, ivec3(textureCoordinate.x, textureCoordinate.y, int( ceil(qt_layers * depthB))), 0);

    // LINEARLY INTERPOLATE BETWEEN THE TWO LAYERS EXTRACTED FROM THE LOOK UP TABLE
    qt_fragColor = pixelA + lambda * (pixelB - pixelA);

    // READ THE GRAY LEVEL FROM THE COLOR TEXTURES RED CHANNEL
    qt_fragColor.a = texelFetch(qt_colorTexture, textureCoordinate, 0).r;

    // APPLY THE LAMBDA FILTER TO DELETE POINTS AS NEEDED
