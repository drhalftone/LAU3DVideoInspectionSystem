...
void main()
{
    ...
    vec4 vertexLo = 16.0 * texelFetch(qt_depthTexture, ivec2(3*col+2,row), 0);
    vec4 vertexMe = 16.0 * texelFetch(qt_depthTexture, ivec2(3*col+0,row), 0);
    vec4 vertexHi = 16.0 * texelFetch(qt_depthTexture, ivec2(3*col+1,row), 0);
    ...
    qt_fragColor.a = 4.0 * magnitudeA; //max(abs(da), abs(db));
