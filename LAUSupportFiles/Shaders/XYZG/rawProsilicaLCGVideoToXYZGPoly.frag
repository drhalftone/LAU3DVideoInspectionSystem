...
void main()
{
    ...
    phase = texture(qt_phaseTexture, phase).r;
    ...
    float cumFlag = snrFlag * mgnFlag * ratFlag * rngFlag;
    ...
    qt_fragColor.a = phase; //max(abs(da), abs(db));
    ...
    //qt_fragColor.x = qt_depthLimits.x;//magnitudeA; //max(abs(da), abs(db));
    //qt_fragColor.y = qt_depthLimits.y;//magnitudeA; //max(abs(da), abs(db));
    //qt_fragColor.z = rngFlag;//magnitudeA; //max(abs(da), abs(db));
}
