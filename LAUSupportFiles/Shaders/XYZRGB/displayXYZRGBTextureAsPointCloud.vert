...
uniform       int qt_arg = 0;        // THIS FLAG TELLS US IF WE ARE IN AUGMENTED REALITY MODE
uniform       int qt_mode;           // THIS FLAG TELLS US TO DISPLY TEXTURE OR SURFACE GRADIENT
uniform     float qt_scale = 30.0f;
...
void main(void)
{
    ...
    vec4 pointA = vec4(texelFetch(qt_texture, ivec2(qt_vertex), 0).rgb, 1.0);

    if (qt_mode == 0){
        // READ THE COLOR VALUE FROM THE TEXTURE
        qt_geometry = texelFetch(qt_texture, ivec2(qt_vertex)+ivec2(1,0), 0);
    } else if (qt_mode == 1){
        // CALCULATE THE SURFACE NORMAL
        vec3 vecAB = (texelFetch(qt_texture, ivec2(qt_vertex)+ivec2(2,0), 0) - pointA).xyz+vec3(0.01, 0.00, 0.00);
        vec3 vecAC = (texelFetch(qt_texture, ivec2(qt_vertex)+ivec2(0,1), 0) - pointA).xyz+vec3(0.00, 0.01, 0.00);
